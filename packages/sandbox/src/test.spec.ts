...
describe("globals?", () => {
  ...
  it("should be able to auto load vue components from goose", () => {
    ...
    expect(wrapper.html()).toMatchInlineSnapshot(
      `"<button class="bg-goose cursor-pointer rounded border-none px-4 py-1 text-white transition duration-200 ease-in-out hover:brightness-95 active:scale-95 active:brightness-75">click me</button>"`,
    );
  });
